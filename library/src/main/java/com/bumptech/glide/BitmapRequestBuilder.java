...
/**
 * A class for creating a request to load a bitmap for an image or from a video. Sets a variety of
 * type independent
...
public class BitmapRequestBuilder<ModelType, TranscodeType>
    ...
    BitmapRequestBuilder(
            LoadProvider<ModelType, ImageVideoWrapper, Bitmap, TranscodeType> loadProvider,
    ...
    /**
     * Load images using the given {@link Downsampler}. Replaces any existing image decoder. Defaults
     * to
     * {@link Downsampler#AT_LEAST}. Will be ignored if the data represented by the model is a video.
     * This replaces any
     * previous calls to {@link #imageDecoder(ResourceDecoder)}  and {@link
    ...
    /**
     * Loads and displays the {@link android.graphics.Bitmap} retrieved by the given thumbnail
     * request
     * if it finishes
     * before this request. Best used for loading thumbnail {@link Bitmap}s that are smaller and will
     * be loaded more
     * quickly than the fullsize {@link Bitmap}. There are no guarantees about the order in which the
     * requests will
     ...
     * example, if
     * you want an animation to occur when either the full {@link android.graphics.Bitmap} loads or
     * the thumbnail
     * loads, you need to call {@link #animate(int)} on both the thumb and the full request. For a
     * simpler thumbnail
     ...
     */
    @Override
    public BitmapRequestBuilder<ModelType, TranscodeType> sizeMultiplier(
     ...
     */
    @Override
    public BitmapRequestBuilder<ModelType, TranscodeType> decoder(
            ResourceDecoder<ImageVideoWrapper, Bitmap> decoder) {
     ...
     */
    @Override
    public BitmapRequestBuilder<ModelType, TranscodeType> encoder(
     ...
     * @see #videoDecoder
     */
    public BitmapRequestBuilder<ModelType, TranscodeType> imageDecoder(
     ...
     * {@link DecodeFormat#PREFER_RGB_565}. This replaces any previous calls to {@link
     ...
     * <p>
     * <p>
     * Note - If using a {@link Transformation} that expect bitmaps to support transparency, this
     * should always be
     * set to ALWAYS_ARGB_8888. RGB_565 requires fewer bytes per pixel and is generally preferable,
     * but it does not
    ...
    public BitmapRequestBuilder<ModelType, TranscodeType> format(DecodeFormat format) {
        ...
        super.cacheDecoder(
                new FileToStreamDecoder<Bitmap>(new StreamBitmapDecoder(downsampler, bitmapPool, format)));
        super.decoder(new ImageVideoBitmapDecoder(imageDecoder, videoDecoder));
        return this;
    }
     ...
     */
    public BitmapRequestBuilder<ModelType, TranscodeType> transform(
     ...
     */
    @Override
    public BitmapRequestBuilder<ModelType, TranscodeType> diskCacheStrategy(
     ...
     */
    @Override
    public BitmapRequestBuilder<ModelType, TranscodeType> sourceEncoder(

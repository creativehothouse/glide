...
import com.bumptech.glide.load.resource.apng.ApngDrawableLoadProvider;
...
public class Glide {
    ...
    Glide(Engine engine, MemoryCache memoryCache, BitmapPool bitmapPool, Context context,
        ...
        dataLoadProviderRegistry.register(ImageVideoWrapper.class, Bitmap.class,
                imageVideoDataLoadProvider);

        GifDrawableLoadProvider gifDrawableLoadProvider =
                new GifDrawableLoadProvider(context, bitmapPool);
        dataLoadProviderRegistry.register(InputStream.class, GifDrawable.class,
                gifDrawableLoadProvider);

        ApngDrawableLoadProvider apngDrawableLoadProvider =
                new ApngDrawableLoadProvider(context, bitmapPool);
        dataLoadProviderRegistry.register(InputStream.class, ApngDrawable.class,
                apngDrawableLoadProvider);
    ...
    /**
     * Returns false if the {@link Glide} singleton has not yet been created and can therefore be
     * setup using
    ...
    /**
     * Creates the {@link Glide} singleton using the given builder. Can be used to set options like
     * cache sizes and
     * locations.
     *
     ...
     * <p>
     * <p>
     * Note that this will only work if {@link View#setTag(Object)} is not called on this view
     * outside
     * of Glide.
     * </p>
     *
    ...
    /**
     * Build a {@link ModelLoader} for the given model class using registered {@link
     ...
     * @param <T>           The type of the model.
     * @param <Y>           The type of the resource.
     * @return A new {@link ModelLoader} for the given model and resource classes, or null if model is
     * null.
    ...
    public static <T, Y> ModelLoader<T, Y> buildModelLoader(T model,
            Class<Y> resourceClass, Context context) {
        return buildModelLoader(model != null ? (Class<T>) model.getClass() : null, resourceClass,
                context);
    }

    /**
     * A method to build a {@link ModelLoader} for the given model that produces {@link InputStream}s
     * using a registered
     * factory.
     *
     ...
     * @see #buildModelLoader(Class, Class, android.content.Context)
     */
    public static <T> ModelLoader<T, ParcelFileDescriptor> buildFileDescriptorModelLoader(
            Class<T> modelClass, Context context) {
     ...
     * <p>
     * <p>
     * Any requests started using a context will only have the application level options applied and
     * will not be
     * started or stopped based on lifecycle events. In general, loads should be started at the level
     * the result
     * will be used in. If the resource will be used in a view in a child fragment,
     * the load should be started with {@link #with(android.app.Fragment)}} using that child
     * fragment.
     * Similarly,
     * if the resource will be used in a view in the parent fragment, the load should be started with
     * {@link #with(android.app.Fragment)} using the parent fragment. In the same vein, if the
     * resource will be used
     * in a view in an activity, the load should be started with {@link
    ...
    /**
     * Begin a load with Glide that will be tied to the given {@link android.app.Activity}'s
     * lifecycle
     * and that uses the
    ...
    /**
     * Begin a load with Glide that will be tied to the given {@link android.app.Fragment}'s
     * lifecycle
     * and that uses
     ...
     * {@link android.graphics.Bitmap} added to the pool must not be currently in use in any other
     * part of the
     * application. Any {@link android.graphics.Bitmap} added to the pool must be removed from the
     * pool before it
     * is added a second time.
     * </p>
     * <p>
     * <p>
     * Note - To make effective use of the pool, any {@link android.graphics.Bitmap} removed from the
     * pool must
     * eventually be re-added. Otherwise the pool will eventually empty and will not serve any useful
     * purpose.
     * </p>
     * <p>
     * <p>
     ...
     * <p>
     * <p>
     * Enough Bitmaps are added to completely fill the pool, so most or all of the Bitmaps currently
     * in the pool will
     * be evicted. Bitmaps are allocated according to the weights of the given sizes, where each size
     * gets
     * (weight / prefillWeightSum) percent of the pool to fill.
     * </p>
     * <p>
     * <p>
     * Note - Pre-filling is done asynchronously using and {@link android.os.MessageQueue.IdleHandler}.
     * Any
     * currently running pre-fill will be cancelled and replaced by a call to this method.
     * </p>
     * <p>
     * <p>
     * This method should be used with caution, overly aggressive pre-filling is substantially worse
     * than not
     * pre-filling at all. Pre-filling should only be started in onCreate to avoid constantly
     * clearing
     * and
     ...
     * MemoryCategory#HIGH} increases
     * Glide's maximum memory usage by up to 50% and {@link MemoryCategory#LOW} decreases Glide's
     * maximum memory
     * usage by 50%. This method should be used to temporarily increase or decrease memory useage for
     * a single
     * Activity or part of the app. Use {@link GlideBuilder#setMemoryCache(MemoryCache)} to set a
     * permanent
    ...
    /**
     * Use the given factory to build a {@link ModelLoader} for models of the given class. Generally
     * the best use of
     * this method is to replace one of the default factories or add an implementation for other
     * similar low level
     ...
     * preferred because it directly links the model with the ModelLoader being used to load it. Any
     * factory replaced
     * by the given factory will have its {@link ModelLoaderFactory#teardown()}} method called.
     * <p>
     * <p>
     * Note - If a factory already exists for the given class, it will be replaced. If that factory
     * is
     * not being
     ...
     * <p>
     * <p>
     * Note - The factory must not be an anonymous inner class of an Activity or another object that
     * cannot be
     * retained statically.
     * </p>
     *
     ...
     */
    public <T, Y> void register(Class<T> modelClass, Class<Y> resourceClass,
    ...
    /**
     * Removes any {@link ModelLoaderFactory} registered for the given model and resource classes if
     * one exists. If a
     * {@link ModelLoaderFactory} is removed, its {@link ModelLoaderFactory#teardown()}} method will
     * be called.
     *
     * @param modelClass The model class.

...
/**
 * A class for managing and starting requests for Glide. Can use activity, fragment and
 * connectivity
 * lifecycle events to
 * intelligently stop, start, and restart requests. Retrieve either by instantiating a new object,
 * or to take advantage
 * built in Activity and Fragment lifecycle handling, use the static Glide.load methods with your
...
public class RequestManager implements LifecycleListener {
     ...
     * starting requests using an {@link android.content.Context} or {@link android.app.Application}
     * to avoid
     ...
     * descendant
     * to this manager based on the Activity/Fragment hierarchy. The hierarchical semantics are
     * identical as for
     ...
     * {@link StreamByteArrayLoader} to fetch an {@link java.io.InputStream} for
     * loading Bitmaps.
     *
     ...
     * <p>
     * <p>
     * Note - this method caches data using only the given String as the cache key. If the data is a
     * Uri outside of
     * your control, or you otherwise expect the data represented by the given String to change
     * without the String
     * identifier changing, Consider using
     * {@link GenericRequestBuilder#signature(Key)} to mixin a signature
     * you create that identifies the data currently at the given String that will invalidate the
     * cache if that data
     ...
     * represented by
     * Uris from some content providers may change without the Uri changing, which means using this
     * method
     * can lead to displaying stale data. Consider using
     * {@link GenericRequestBuilder#signature(Key)} to mixin a signature
     * you create based on the data at the given Uri that will invalidate the cache if that data
     * changes.
     ...
     * the given uri if
     * available and uses the given additional data to build a unique signature for cache
     * invalidation.
     *
     ...
     */
    @Deprecated
    public DrawableTypeRequest<Uri> loadFromMediaStore(Uri uri, String mimeType,
     ...
     * {@link Uri}s if the given uri is not a media store uri or if no pre-generated thumbnail exists
     * for the given
     * uri.
     * </p>
     * <p>
     * <p>
     * Note - This method by default caches data using the given Uri as the key. Since content in the
     * media store
    ...
    public DrawableTypeRequest<Uri> fromMediaStore() {
        ModelLoader<Uri, InputStream> genericStreamLoader =
                Glide.buildStreamModelLoader(Uri.class, context);
        ModelLoader<Uri, InputStream> mediaStoreLoader =
                new MediaStoreStreamLoader(context, genericStreamLoader);
        ...
        return optionsApplier.apply(
                new DrawableTypeRequest<Uri>(Uri.class, mediaStoreLoader, fileDescriptorModelLoader,
     ...
     * registered for {@link File} to load the image represented by the given {@link File}. Defaults
     * to
     * {@link StreamFileLoader.Factory} and
     * {@link StreamFileLoader} to load images from {@link File}s.
     * <p>
     * <p>
     * Note - this method caches data for Files using only the file path itself as the cache key. The
     * data in the
     * File can change so using this method can lead to displaying stale data. If you expect the data
     * in the File to
     * change, Consider using
     * {@link GenericRequestBuilder#signature(Key)} to mixin a signature
     * you create that identifies the data currently in the File that will invalidate the cache if
     * that data
     * changes. Alternatively, using {@link DiskCacheStrategy#NONE} and/or
     * {@link DrawableRequestBuilder#skipMemoryCache(boolean)} may be appropriate.
     * </p>
    ...
    /**
     * Returns a request builder that uses the {@link ModelLoaderFactory} currently
     * registered for {@link Integer} to load the image represented by the given {@link Integer}
     * resource id. Defaults
     * to {@link StreamResourceLoader.Factory} and
     * {@link StreamResourceLoader} to load resource id models.
     * <p>
     * <p>
     * By default this method adds a version code based signature to the cache key used to cache this
     * resource in
     * Glide. This signature is sufficient to guarantee that end users will see the most up to date
     * versions of
     * your Drawables, but during development if you do not increment your version code before each
     * install and
     * you replace a Drawable with different data without changing the Drawable name, you may see
     * inconsistent
    ...
    public DrawableTypeRequest<Integer> fromResource() {
        return (DrawableTypeRequest<Integer>) loadGeneric(Integer.class).signature(
     ...
     * {@link HttpUrlFetcher} to load {@link java.net.URL} models.
     *
     * @see #from(Class)
    ...
    /**
     * Returns a request builder that uses a {@link StreamByteArrayLoader} to load an image from the
     * given byte array.
     * <p>
     * <p>
     * <p>
     ...
     * @param id    A unique id that identifies the image represented by the model suitable for use as a
     *              cache key
    ...
    public DrawableTypeRequest<byte[]> fromBytes() {
        return (DrawableTypeRequest<byte[]>) loadGeneric(byte[].class).signature(
     ...
     * registered for the given model class for {@link InputStream}s and {@link
     * ParcelFileDescriptor}s
     * to load a
     * thumbnail from either the image or the video represented by the given model.
     * <p>
     * <p>
     * Note - for maximum efficiency, consider using {@link #from(Class)}} to avoid repeatedly
     * allocating builder
     * objects.
     * </p>
     *
     ...
     * {@link java.io.InputStream}s and {@link android.os.ParcelFileDescriptor}s to load a thumbnail
     * from objects of
     ...
     * also use this object for repeated loads by calling <code>request.load(model).into(target)</code>.
     * You may
    ...
    private <T> DrawableTypeRequest<T> loadGeneric(Class<T> modelClass) {
        ...
        return optionsApplier.apply(
                new DrawableTypeRequest<T>(modelClass, streamModelLoader, fileDescriptorModelLoader,
                        context, glide, requestTracker, lifecycle, optionsApplier));
    }

    /**
     * An interface that allows a default set of options to be applied to all requests started from
     * an
     * {@link RequestManager}.
     */
    ...
    public final class VideoModelRequest<T> {
        ...
        public DrawableTypeRequest<T> load(T model) {
            return (DrawableTypeRequest<T>) optionsApplier.apply(
                    new DrawableTypeRequest<T>(getSafeClass(model), null, loader, context, glide,
    ...
    public final class ImageModelRequest<T> {
        ...
        public DrawableTypeRequest<T> from(Class<T> modelClass) {
            return optionsApplier.apply(
                    new DrawableTypeRequest<T>(modelClass, loader, null, context, glide, requestTracker,
    ...
    /**
     * A helper class for building requests with custom {@link ModelLoader}s that requires the user
     * to
     * provide a
     * specific model.
     *
    ...
    public final class GenericModelRequest<A, T> {
        ...
        public final class GenericTypeRequest {
            ...
            public <Z> GenericTranscodeRequest<A, T, Z> as(Class<Z> resourceClass) {
                GenericTranscodeRequest<A, T, Z> result = optionsApplier.apply(
                        new GenericTranscodeRequest<A, T, Z>(context, glide, modelClass, modelLoader, dataClass,

...
public interface Transformation<T> {
     ...
     * <p>
     * <p>
     * Note - If the original resource object is not returned, the original resource will be recycled
     * and it's
     * internal resources may be reused. This means it is not safe to rely on the original resource
     * or
     * any internal
     * state of the original resource in any new resource that is created. Usually this shouldn't
     * occur, but if
     * absolutely necessary either the original resource object can be returned with modified
     * internal
     * state, or
    ...
    /**
     * A method to get a unique identifier for this particular transformation that can be used as
     * part
     * of a cache key.
     * The fully qualified class name for this class is appropriate if written out, but
     * getClass().getName() is not
     * because the name may be changed by proguard.
     * <p>
     * <p>
     * If this transformation does not affect the data that will be stored in cache, returning an
     * empty string here
     * is acceptable.
     * </p>
     *

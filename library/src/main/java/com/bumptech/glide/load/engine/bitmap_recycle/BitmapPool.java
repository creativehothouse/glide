...
public interface BitmapPool {
    ...
    /**
     * Multiplies the initial size of the pool by the given multipler to dynamically and
     * synchronously
     * allow users to
     ...
     * <p>
     * <p>
     * Note - If the {@link android.graphics.Bitmap} is rejected (this method returns false) then it
     * is the caller's
     * responsibility to call {@link android.graphics.Bitmap#recycle()}.
     * </p>
     * <p>
     * <p>
     * Note - This method will return {@code true} if the given {@link android.graphics.Bitmap} is
     * synchronously
     * evicted after being accepted. The only time this method will return {@code false} is if the
     * {@link android.graphics.Bitmap} is not eligible to be added to the pool (either it is not
     * mutable or it is
     ...
     * configuration, and containing
     * only transparent pixels or null if no such {@link android.graphics.Bitmap} could be obtained
     * from the pool.
     * <p>
     * <p>
     * Because this method erases all pixels in the {@link Bitmap}, this method is slightly slower
     * than
     * {@link #getDirty(int, int, android.graphics.Bitmap.Config)}. If the {@link
     * android.graphics.Bitmap} is being
     * obtained to be used in {@link android.graphics.BitmapFactory} or in any other case where every
     * pixel in the
     * {@link android.graphics.Bitmap} will always be overwritten or cleared,
     * {@link #getDirty(int, int, android.graphics.Bitmap.Config)} will be faster. When in doubt, use
     * this method
     ...
     * it should be used with caution and only when the caller is sure that they are going to erase
     * the
     ...
     * random image data or null if no such {@link android.graphics.Bitmap} could be obtained from
     * the
     * pool.
     ...
     *
     * @param level The level from {@link android.content.ComponentCallbacks2} to use to determine
     * how
     * many

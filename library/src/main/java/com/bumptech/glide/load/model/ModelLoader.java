 ...
 * specific size.
 * <p>
 * This not only avoids having to duplicate dimensions in xml and in your code in order to
 * determine
 * the size of a
 * view on devices with different densities, but also allows you to use layout weights or otherwise
 * programatically set the dimensions of the view without forcing you to fetch a generic resource
 * size.
 * <p>
...
public interface ModelLoader<T, Y> {
     ...
     * <p>
     * <p>
     * Note - If no valid data fetcher can be returned (for example if a model has a null URL), then
     * it is
     * acceptable to return a null data fetcher from this method. Doing so will be treated any other
     * failure or

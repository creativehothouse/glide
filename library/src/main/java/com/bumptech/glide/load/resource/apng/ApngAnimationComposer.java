...
import net.ellerton.japng.android.api.PngAnimationComposer;
...
/**
 * Takes loaded APNG frames and composes them into an android AnimationDrawable.
 */
class ApngAnimationComposer {
    /**
    ...
    private List<PngAnimationComposer.Frame> frames;
    ...
    public ApngAnimationComposer(Resources resources, PngHeader header,
        ...
        this.frames = new ArrayList<PngAnimationComposer.Frame>(animationControl.numFrames);
    ...
    public static Bitmap getReferenceImage(Resources resources) {
        if (referenceImage == null) {
            referenceImage = Bitmap.createBitmap(1, 1, Bitmap.Config.ARGB_8888);
        }
    ...
    public ApngDrawable assemble() {
        ...
        for (int i = 0; i < repeatSequenceCount; i++) {
            for (PngAnimationComposer.Frame frame : frames) {
    ...
    public void completeFrame(Argb8888Bitmap frameImage) {
        ...
        frames.add(new PngAnimationComposer.Frame(currentFrame, d));

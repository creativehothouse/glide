...
public interface BitmapDecoder<T> {
     ...
     */
    Bitmap decode(T resource, BitmapPool bitmapPool, int outWidth, int outHeight,
            DecodeFormat decodeFormat) throws Exception;

    /**
     * Returns some unique String id that distinguishes this decoder from any other decoder.
     * <p>
     * <p>
     * This method can return the empty string if for all practical purposes it applies no
     * transformations to the
     * data while loading the resource. For {@link android.graphics.Bitmap}s this would mean at a
     * minimum doing no
